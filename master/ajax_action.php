...
if (isset($_POST['page'])) {
	if ($_POST['page'] == 'register') {
		...
		if ($_POST['action'] == 'register') {
			...
			This is a verification eMail, please click the link to verify your eMail address by clicking this ' . $exam->home_page . 'verify_email.php?type=master&code=' . $admin_verification_code . '
			In case if you have any difficulty please eMail us.
			Thank you,
		...
		if ($_POST['action'] == 'fetch') {
			...
			foreach ($result as $row) {
				...
				if ($row['online_exam_status'] == 'Pending') {
					$status = '<h5><span class="badge badge-warning" style="font-weight:normal !important;">Pending</span></h5>';
				}

				if ($row['online_exam_status'] == 'Created') {
					$status = '<h5><span class="badge badge-success" style="font-weight:normal !important;">Created</span></h5>';
				}

				if ($row['online_exam_status'] == 'Started') {
					$status = '<h5><span class="badge badge-primary" style="font-weight:normal !important;">Started</span></h5>';
				}

				if ($row['online_exam_status'] == 'Completed') {
					$status = '<h5><span class="badge badge-dark" style="font-weight:normal !important;">Completed</span></h5>';
				}

				if ($exam->Is_exam_is_not_started($row["online_exam_id"])) {
					$edit_button = '
					<button type="button" name="edit" class="btn btn-success btn-sm edit" id="' . $row['online_exam_id'] . '"><i class="fa fa-edit"></i></button>
					';

					$delete_button = '<button type="button" name="delete" class="btn btn-danger btn-sm delete" id="' . $row['online_exam_id'] . '"><i class="fa fa-trash"></i></button>';
				} else {
				...
				if ($exam->Is_allowed_add_question($row['online_exam_id'])) {
					...
					$question_button = '
					<a href="question.php?code=' . $row['online_exam_code'] . '" class="btn btn-success btn-sm">View Questions</a>
					';
				}
		...
		if ($_POST['action'] == 'fetch') {
			...
			foreach ($result as $row) {
				...
				if ($exam->Is_exam_is_not_started($exam_id)) {
					$edit_button = '<button type="button" name="edit" class="btn btn-success btn-sm edit" id="' . $row['question_id'] . '"><i class="fa fa-edit"></i></button>';

					$delete_button = '<button type="button" name="delete" class="btn btn-danger btn-sm delete" id="' . $row['question_id'] . '"><i class="fa fa-trash"></i></button>';
				}
	...
	if ($_POST['page'] == 'user') {
		if ($_POST['action'] == 'fetch') {
			...
			foreach ($result as $row) {
				...
				if ($row["user_email_verified"] == 'yes') {
					$is_email_verified = '<h4><label class="badge badge-success" style="font-weight:normal !important;">Yes</label></h4>';
				} else {
					$is_email_verified = '<h5><label class="badge badge-danger" style="font-weight:normal !important;">No</label></h5>';
				}

				$sub_array[] = $is_email_verified;
				$sub_array[] = '<button type="button" name = "view_detail" class="btn btn-primary btm-sm details" id="' . $row["user_id"] . '">View Details</button>';
				$data[] = $sub_array;
			}
		...
		if ($_POST['action'] == 'fetch_data') {
			...
			foreach ($result as $row) {
				$is_email_verified = '';
				if ($row["user_email_verified"] == 'yes') {
					$is_email_verified = '<h5><label class="badge badge-success" style="font-weight:normal !important;">Email Verified</label></h5>';
				} else {
					$is_email_verified = '<h5><label class="badge badge-danger" style="font-weight:normal !important;">Email Not Verified</label></h5>';
				}
	...
	if ($_POST['page'] == 'exam_enroll') {
		if ($_POST['action'] == 'fetch') {
			...
			foreach ($result as $row) {
				$sub_array = array();
				$sub_array[] = "<img src='../upload/" . $row["user_image"] . "' class='img-fluid img-thumbnail' width='75' />";
				$sub_array[] = $row["user_name"];
				$sub_array[] = $row["user_gender"];
				$sub_array[] = $row["user_mobile_no"];
				$is_email_verified = '';
				...
				if ($row['user_email_verified'] == 'yes') {
					$is_email_verified = '<h5><label class="badge badge-success" style="font-weight:normal !important;">Yes</label></h5>';
				} else {
					$is_email_verified = '<h5><label class="badge badge-danger" style="font-weight:normal !important;">No</label></h5>';
				}
				$sub_array[] = $is_email_verified;
